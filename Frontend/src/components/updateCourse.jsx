...
export default function UpdateCourse(){
    ...
    let isUpdate = async(data) =>{
        courseBox.current.value = data.course_name
    ...
    let doneupdate = async(event) =>{
        ...
        let obj = {
            course: courseBox.current.value,
        ...
        let response = await webmethod.putapiWthToken(apis.updateCourse + "/" + ID,obj,loginStatus.token)
    ...
    <table className="table table-striped table-bordered table-hover table-responsive-md container">
        ...
        <tbody>
            {
                list.filter((obj)=> search.toLowerCase() === '' ? (obj.is_active && obj):(obj.is_active && obj.course_name.toLowerCase().includes(search))).map(obj=><tr>
